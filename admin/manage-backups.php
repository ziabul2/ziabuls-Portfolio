...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($action === 'restore' && $filename) {
        ...
        $filePath = getPortfolioFilePath();
        $backupManager = new BackupManager($filePath);
        $keepCount = isset($_POST['keep_count']) ? (int)$_POST['keep_count'] : 2;
        $deletedCount = $backupManager->cleanupOldBackups($keepCount);
        setFlashMessage("Cleanup complete. Deleted $deletedCount old backup(s).", 'success');
        header('Location: manage-backups.php');
        exit;
...
// Handle GET actions that output files directly (so headers don't get sent)
if ($action === 'export_all') {
    $filePath = getPortfolioFilePath();
    $backupManager = new BackupManager($filePath);
    $zipFile = $backupManager->exportAllBackups();
    ...
    if ($zipFile && file_exists($zipFile)) {
        ...
        setFlashMessage('Failed to create ZIP archive. Ensure ZipArchive extension is enabled.', 'error');
        header('Location: manage-backups.php');
        exit;
    }
...
<!-- Handle Create Backup (Moved to Top) -->
                    ...
                    <?php foreach ($backups as $backup): ?>
                        <tr style="border-bottom: 1px solid #444; transition: 0.2s;" onmouseover="this.style.backgroundColor='rgba(198, 120, 221, 0.1)'" onmouseout="this.style.backgroundColor='transparent'">
                            ...
                            <td style="padding: 12px; text-align: center;">
                                ...
                                <a href="manage-backups.php?action=download&filename=<?php echo urlencode($backup['filename']); ?>" class="btn-action" style="background: #98c379; color: white; padding: 6px 12px; border: none; border-radius: 3px; cursor: pointer; margin: 2px; transition: 0.2s; display: inline-block; text-decoration: none;" title="Download backup">
